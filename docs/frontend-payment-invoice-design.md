...
- 第一阶段充值使用微信支付 Native / JSAPI，并接入微信支付凭证页“开发票”入口，按充值订单金额开票。
- 第二阶段再补站内发票中心，按“已实际消费金额”申请，避免充值后退款、赠送额度和重复开票的问题。
...
- 产品已决定第一阶段按充值订单开票；第二阶段再补按实际消费开票。财务仍需最终确认税目、税率和退款冲红流程。
...
| --- | ---: | --- | --- |
| 8 元 | 8 次 | 第一阶段按充值订单开票 | 试用和低频用户 |
| 18 元 | 18 次 | 第一阶段按充值订单开票 | 普通个人用户 |
| 38 元 | 38 次 | 第一阶段按充值订单开票 | 小团队 |
| 88 元 | 88 次 | 第一阶段按充值订单开票 | 高频用户/API 用户 |
...
- 产品决策调整为先走微信凭证页充值订单开票，再走消费后开票。必须实现微信抬头通知、获取抬头、开票、下载和退款冲红限制后，再打开 `support_fapiao=true`。
- 如果微信支付账号走服务商/子商户链路，先用服务商 API 检查子商户开票状态；`NO_AUTH` 常见原因是子商户未接受邀请或未完成“服务商电子发票”开票模式配置。
- 如果要让用户最大程度自助开票，推荐接入微信支付电子发票的自建/第三方模式或独立电子发票服务商，由系统自动校验可开票金额、自动开票、自动回填下载链接。

电子发票服务：

- 第一阶段：接微信支付凭证页“开发票”入口，按充值订单金额开电子发票。
- 第二阶段：增加站内发票中心，按实际消费金额申请开票。
...
3. 实现钱包页和微信充值下单。
4. 支付回调验签、解密、幂等入账。
5. 第一阶段接入微信支付充值订单开票：抬头通知、获取抬头、开票、下载、退款冲红限制。
6. 第一阶段验收通过后打开 `WECHAT_PAY_SUPPORT_FAPIAO=true`。
7. `POST /api/v1/jobs` 改为余额冻结后创建任务。
8. Worker 成功完成后正式扣费，系统失败释放冻结。
9. 后续增加站内发票中心，按实际消费金额申请开票。
