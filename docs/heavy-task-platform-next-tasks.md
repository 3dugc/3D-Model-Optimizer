...
- [x] 完成微信登录、余额充值、`1 元/次` 扣费和发票中心产品/技术设计。
  - 设计文档：`docs/frontend-payment-invoice-design.md`。
- [ ] 准备微信支付商户号。
- [ ] 准备微信支付 AppID。
- [ ] 准备微信支付商户私钥。
- [ ] 准备微信支付证书序列号。
- [ ] 准备微信支付 API v3 key。
- [x] 实现微信登录能力代码。
  - [x] 后端支持微信 OAuth 授权跳转和回调。
  - [x] 回调后保存 `openid`，若微信返回 `unionid` 则同步保存，便于后续小程序共享账户。
  - [x] Web UI 支持真实微信登录按钮、登录 token 回收和开发模拟登录兜底。
  - [x] 支持公众号网页授权 `offiaccount` 和开放平台网站扫码 `website` 两种模式。
- [ ] 完成微信登录生产配置。
  - [ ] 在 Portainer 配置 `WECHAT_OAUTH_APP_SECRET`。
  - [ ] 确认 `WECHAT_OAUTH_APP_ID` 使用已绑定商户号的公众号或开放平台网站应用。
  - [ ] 公众号后台配置网页授权域名 `3dugc.com`。
  - [ ] 如需桌面端扫码登录，申请并配置微信开放平台网站应用，切换 `WECHAT_LOGIN_MODE=website` 和 `WECHAT_OAUTH_SCOPE=snsapi_login`。
  - [ ] 用真实微信账号登录后确认同一 `unionid` 可和后续小程序账户打通。
- [x] 实现 Web 用户表和微信 `openid/unionid` 绑定。
- [x] 实现用户钱包、充值订单、余额流水和任务扣费流水。
- [x] 实现充值档位：`10`、`30`、`50`、`100` 元。
- [x] 实现任务创建时冻结 `100` 分余额。
- [x] 实现任务成功后正式扣费 `100` 分。
- [x] 实现系统失败时释放冻结余额。
- [ ] 实现任务未开始取消时释放冻结余额。
- [x] 在现有 Web UI 增加账户、余额和充值面板。
- [x] 定义 `PaymentProvider` 接口。
- [x] 实现微信 Native 下单。
- [x] 创建订单时返回微信 Native `code_url`。
- [ ] Web UI 将 `code_url` 渲染成扫码二维码。
- [x] 实现微信支付回调验签。
- [x] 实现微信支付回调解密。
- [x] 微信支付回调验签失败时拒绝处理。
- [x] 微信支付成功后订单幂等更新为 `paid`。
...
- [ ] 增加支付回调幂等测试。
- [ ] 完成微信支付沙箱或测试商户回调测试。
- [ ] 实现发票中心：可开票金额、发票抬头、申请记录、下载入口。
- [ ] 发票金额按“已消费现金金额 - 已开票金额 - 已退款金额”计算。
- [ ] 自动开票建议满 `10` 元，小额消费可累计，用户明确索取时保留人工开票入口。
- [ ] 第一阶段支持人工开数电普通发票并回填下载链接。
