...
微信扫码付费使用微信支付 **Native 支付**：

1. 用户或外部系统创建任务后，Control API 计算价格并创建订单。
2. 调用微信支付 Native 下单接口，获取 `code_url`。
3. Web UI 把 `code_url` 转成二维码展示。
4. 微信支付异步回调到 `POST /api/v1/payments/wechat/notify`。
5. 服务端验签、解密通知、幂等更新订单为 `paid`。
6. 订单支付成功后，任务才允许进入 `queued`。
