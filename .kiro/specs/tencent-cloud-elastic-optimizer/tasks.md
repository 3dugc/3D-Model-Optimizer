...
- [ ] 10. 接入微信 Native 支付
  - [x] 10.1 完成微信登录、余额充值、`1 元/次` 扣费和发票中心产品/技术设计
  - [x] 10.2 接入微信登录代码，支持桌面扫码和微信内网页授权
    - [x] 10.2.1 新增微信 OAuth 授权跳转和回调
    - [x] 10.2.2 回调后保存 `openid/unionid`，为小程序共享账户预留
    - [x] 10.2.3 Web UI 接收登录 token 并保留开发模拟登录
    - [ ] 10.2.4 在 Portainer 配置 `WECHAT_OAUTH_APP_SECRET`
    - [ ] 10.2.5 在公众号后台配置网页授权域名 `3dugc.com`
    - [ ] 10.2.6 如需桌面扫码，申请微信开放平台网站应用并切换 `WECHAT_LOGIN_MODE=website`
  - [x] 10.3 实现 Web 用户表和微信 `openid/unionid` 绑定
  - [x] 10.4 实现用户钱包、充值订单、余额流水和任务扣费流水
  - [x] 10.5 实现充值档位：`10`、`30`、`50`、`100` 元
  - [x] 10.6 定义 PaymentProvider 接口
  - [x] 10.7 实现 Wechat Native 下单
  - [ ] 10.8 Web UI 展示二维码
  - [x] 10.9 实现微信支付回调验签和解密
  - [x] 10.10 支付成功后触发 Job 入队
  - [ ] 10.11 实现订单过期、关闭和退款状态处理
  - [ ] 10.12 增加支付回调幂等测试
  - [x] 10.13 任务创建时冻结 `100` 分，任务成功后正式扣费
  - [x] 10.14 系统失败时释放冻结余额
  - [ ] 10.14a 任务未开始取消时释放冻结余额
  - [ ] 10.15 实现发票中心：可开票金额、抬头、申请记录、下载入口
  - [ ] 10.16 支持人工开数电普通发票并回填下载链接
